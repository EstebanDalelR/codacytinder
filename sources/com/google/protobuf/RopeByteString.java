...
final class RopeByteString extends ByteString {
    ...
    static ByteString concatenate(ByteString byteString, ByteString byteString2) {
        ...
        if (byteString instanceof RopeByteString) {
            RopeByteString ropeByteString = (RopeByteString) byteString;
            if (ropeByteString.right.size() + byteString2.size() < ProfileEditingConfig.DEFAULT_MAX_LENGTH) {
    ...
    private boolean equalsFragments(ByteString byteString) {
        ...
        while (true) {
            boolean equalsRange;
            int size = byteString2.size() - byteString;
            int size2 = byteString3.size() - i;
            int min = Math.min(size, size2);
            if (byteString == null) {
                equalsRange = byteString2.equalsRange(byteString3, i, min);
            } else {
                equalsRange = byteString3.equalsRange(byteString2, byteString, min);
            }
            if (!equalsRange) {
                return false;
            }

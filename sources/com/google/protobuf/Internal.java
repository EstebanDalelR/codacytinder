...
public final class Internal {
    ...
    public static class MapAdapter<K, V, RealValue> extends AbstractMap<K, V> {
        ...
        private class EntryAdapter implements Entry<K, V> {
            ...
            public V setValue(V v) {
                v = this.realEntry.setValue(MapAdapter.this.valueConverter.doBackward(v));
                if (v == null) {
                    return null;
                }
                return MapAdapter.this.valueConverter.doForward(v);
            }
        }
        ...
        public static <T extends EnumLite> Converter<Integer, T> newEnumConverter(final EnumLiteMap<T> enumLiteMap, final T t) {
            return new Converter<Integer, T>() {
                public T doForward(Integer num) {
                    num = enumLiteMap.findValueByNumber(num.intValue());
        ...
        public V get(Object obj) {
            obj = this.realMap.get(obj);
            if (obj == null) {
                return null;
            }
            return this.valueConverter.doForward(obj);
        }

        public V put(K k, V v) {
            k = this.realMap.put(k, this.valueConverter.doBackward(v));
            if (k == null) {
                return null;
            }
            return this.valueConverter.doForward(k);
        }
    ...
    public static int hashCodeByteBuffer(ByteBuffer byteBuffer) {
        if (byteBuffer.hasArray()) {
            byteBuffer = partialHash(byteBuffer.capacity(), byteBuffer.array(), byteBuffer.arrayOffset(), byteBuffer.capacity());
            if (byteBuffer == null) {
                byteBuffer = true;
            }
            return byteBuffer;
        ...
        duplicate.clear();
        byteBuffer = byteBuffer.capacity();
        while (duplicate.remaining() > 0) {
            int remaining = duplicate.remaining() <= i ? duplicate.remaining() : i;
            duplicate.get(bArr, 0, remaining);
            byteBuffer = partialHash(byteBuffer, bArr, 0, remaining);
        }
        if (byteBuffer == null) {
            byteBuffer = true;
        }

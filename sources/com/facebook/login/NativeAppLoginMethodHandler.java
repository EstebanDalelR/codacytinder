...
abstract class NativeAppLoginMethodHandler extends LoginMethodHandler {
    ...
    private Result handleResultOk(Request request, Intent intent) {
        ...
        String obj = intent.get(ManagerWebServices.PARAM_ERROR_CODE) != null ? intent.get(ManagerWebServices.PARAM_ERROR_CODE).toString() : null;
        String errorMessage = getErrorMessage(intent);
        String string = intent.getString("e2e");
        ...
        } else if (C1747v.f4823a.contains(error) != null) {
            return null;
        } else {
            if (C1747v.f4824b.contains(error) != null) {
    ...
    private Result handleResultCancel(Request request, Intent intent) {
        ...
        if ("CONNECTION_FAILURE".equals(obj)) {
            return Result.createErrorResult(request, error, getErrorMessage(intent), obj);
        }

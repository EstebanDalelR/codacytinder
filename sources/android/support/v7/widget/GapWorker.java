...
final class GapWorker implements Runnable {
    ...
    static Comparator<Task> sTaskComparator = new C07061();
    ...
    static class C07061 implements Comparator<Task> {
        C07061() {
        }
    ...
    private void prefetchInnerRecyclerViewWithDeadline(@Nullable RecyclerView recyclerView, long j) {
        if (recyclerView != null) {
            ...
            if (layoutPrefetchRegistryImpl.mCount != 0) {
                try {
                    C0529b.m1953a("RV Nested Prefetch");
    ...
    private void flushTaskWithDeadline(Task task, long j) {
        task = prefetchPositionWithDeadline(task.view, task.position, task.immediate ? Format.OFFSET_SAMPLE_RELATIVE : j);

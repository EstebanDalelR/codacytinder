...
public class ToolbarWidgetWrapper implements DecorToolbar {
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean z) {
        this(toolbar, z, C0667R.string.abc_action_bar_up_description, C0667R.drawable.abc_ic_ab_back_material);
    }
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean z, int i, int i2) {
        ...
        toolbar = TintTypedArray.obtainStyledAttributes(toolbar.getContext(), null, C0667R.styleable.ActionBar, C0667R.attr.actionBarStyle, 0);
        this.mDefaultNavigationIcon = toolbar.getDrawable(C0667R.styleable.ActionBar_homeAsUpIndicator);
        if (z) {
            z = toolbar.getText(C0667R.styleable.ActionBar_title);
            if (!TextUtils.isEmpty(z)) {
                setTitle(z);
            }
            z = toolbar.getText(C0667R.styleable.ActionBar_subtitle);
            if (!TextUtils.isEmpty(z)) {
                setSubtitle(z);
            }
            Drawable drawable = toolbar.getDrawable(C0667R.styleable.ActionBar_logo);
            if (drawable == true) {
                setLogo(drawable);
            }
            drawable = toolbar.getDrawable(C0667R.styleable.ActionBar_icon);
            if (drawable == true) {
                setIcon(drawable);
            }
            ...
            setDisplayOptions(toolbar.getInt(C0667R.styleable.ActionBar_displayOptions, 0));
            z = toolbar.getResourceId(C0667R.styleable.ActionBar_customNavigationLayout, 0);
            if (z) {
                setCustomView(LayoutInflater.from(this.mToolbar.getContext()).inflate(z, this.mToolbar, false));
                setDisplayOptions(this.mDisplayOpts | 16);
            }
            z = toolbar.getLayoutDimension(C0667R.styleable.ActionBar_height, 0);
            if (z <= false) {
                LayoutParams layoutParams = this.mToolbar.getLayoutParams();
                layoutParams.height = z;
                this.mToolbar.setLayoutParams(layoutParams);
            }
            z = toolbar.getDimensionPixelOffset(C0667R.styleable.ActionBar_contentInsetStart, -1);
            int dimensionPixelOffset = toolbar.getDimensionPixelOffset(C0667R.styleable.ActionBar_contentInsetEnd, -1);
            if (z < false || dimensionPixelOffset >= 0) {
                this.mToolbar.setContentInsetsRelative(Math.max(z, 0), Math.max(dimensionPixelOffset, 0));
            }
            z = toolbar.getResourceId(C0667R.styleable.ActionBar_titleTextStyle, 0);
            if (z) {
                this.mToolbar.setTitleTextAppearance(this.mToolbar.getContext(), z);
            }
            z = toolbar.getResourceId(C0667R.styleable.ActionBar_subtitleTextStyle, 0);
            if (z) {
                this.mToolbar.setSubtitleTextAppearance(this.mToolbar.getContext(), z);
            }
            z = toolbar.getResourceId(C0667R.styleable.ActionBar_popupTheme, 0);
            if (z) {
        ...
        this.mToolbar.setNavigationOnClickListener(new C07361());
    }
    ...
    public void setMenu(Menu menu, MenuPresenter.Callback callback) {
        if (this.mActionMenuPresenter == null) {
            ...
            this.mActionMenuPresenter.setId(C0667R.id.action_menu_presenter);
        }
    ...
    private void ensureSpinner() {
        if (this.mSpinner == null) {
            this.mSpinner = new AppCompatSpinner(getContext(), null, C0667R.attr.actionDropDownStyle);
    ...
    public C0609n setupAnimatorToVisibility(final int i, long j) {
        return ViewCompat.m2221m(this.mToolbar).m2377a(i == 0 ? 1.0f : 0.0f).m2378a(j).m2379a(new C2899o() {
    ...
    public void setBackgroundDrawable(Drawable drawable) {
        ViewCompat.m2183a(this.mToolbar, drawable);
    }
